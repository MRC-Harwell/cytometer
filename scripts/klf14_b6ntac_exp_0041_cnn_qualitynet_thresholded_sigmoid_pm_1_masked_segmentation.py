...
for i_fold, idx_test in enumerate(idx_orig_test_all):
    ...
    # multiply histology by -1/+1 segmentation mask
    train_aux = 2 * (train_onecell_testlab.astype(np.float32) - 0.5)
    train_onecell_im *= np.repeat(train_aux, repeats=train_onecell_im.shape[3], axis=3)
    ...
    test_onecell_im *= np.repeat(test_aux, repeats=test_onecell_im.shape[3], axis=3)
