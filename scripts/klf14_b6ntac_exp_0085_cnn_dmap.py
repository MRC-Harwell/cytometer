...
experiment_id = 'klf14_b6ntac_exp_0085_cnn_dmap'
...
for i_fold, idx_test in enumerate(idx_test_all):
    ...
    # number of images
    n_train = train_dataset['im'].shape[0]
    n_test = test_dataset['im'].shape[0]
    ...
    print('Train images: ' + str(n_train) + ' (full), '
          + str(train_dataset['im'].shape[0]) + ' (minus small masks)')
    print('Test images: ' + str(n_test) + ' (full), '
          + str(test_dataset['im'].shape[0]) + ' (minus small masks)')
