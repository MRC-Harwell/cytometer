    ...
    if os.path.isfile(dataframe_areas_filename):
        ...
        for depot in ['sqwat', 'gwat']:
            ...
            for i_file, json_file in enumerate(json_annotation_files):
                ...
                # open full resolution histology slide
                histo_file = json_file.replace('_exp_0106_' + method + '_aggregated.json', '.ndpi')
                ...
                with np.load(coarse_mask_file) as aux:
                    ...
                    if DEBUG:
                        ...
                        plt.imshow(foo)
                        plt.title(os.path.basename(histo_file))
...
########################################################################################################################
## Heatmaps of whole slides (Klf14)
########################################################################################################################
...
for i_file, json_file in enumerate(json_annotation_files):
    ...
    kernel_file = os.path.splitext(histo_file)[0]
...
########################################################################################################################
## Heatmaps of whole slides (GTex)
##
# NOTE: Because the 16x downsampling is fine for segmentation, but too coarse with these images for generating heatmaps,
# we are going instead to take screenshots from AIDA, which look nicer and don't leave gaps due to undersampling
########################################################################################################################

# paths
histo_dir = os.path.join(home, 'jesse_mousedata/GTEx')
...
for i_file, histo_file in enumerate(histo_files_list):
    ...
    # name of corresponding annotations file
    kernel_file = os.path.splitext(histo_file)[0]
    json_file = kernel_file + '_exp_0001_corrected_aggregated.json'
    ...
    # name of file where rough mask was saved to
    coarse_mask_file = os.path.basename(histo_file)
    coarse_mask_file = coarse_mask_file.replace('.svs', '_rough_mask.npz')
    coarse_mask_file = os.path.join(annotations_dir, coarse_mask_file)
    ...
    # load list of contours in Corrected segmentations
    json_file_corrected = json_file.replace('_exp_0001_corrected_aggregated.json', '_exp_0001_corrected.json')
    json_file_corrected = os.path.join(annotations_dir, os.path.basename(json_file_corrected))
    cells, props = cytometer.data.aida_get_contours(json_file_corrected, layer_name='White adipocyte.*', return_props=True)
    ...
    for c in cells:
        ...
        # convert to downsampled coordinates
        c = np.array(c)
        c_down = c / downsample_factor_actual
        ...
        centroid = np.mean(c, axis=0)
        centroids_all.append(centroid)
        centroids_down_all.append(centroid / downsample_factor_actual)

        # add object described by contour to areas_mask
        draw.polygon(list(c_down.flatten()), outline="white", fill="white")

    # convert mask from RGBA to binary
    areas_mask = np.array(areas_mask).astype(bool)
    ...
    # convert area values to quantiles using the male colourmap
    quantiles_grid = f_area2quantile_m(quantiles_grid)
