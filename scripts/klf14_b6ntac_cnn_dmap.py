...
# augment data, using the same seed so that all correponding images, dmaps and masks undergo
...
if DEBUG:
    for i in range(im_augmented.shape[0]):
        plt.clf()
        plt.subplot(221)
...
# transformations
seed = 1
dmap_datagen.fit(dmap, augment=True, seed=seed)
im_datagen.fit(im, augment=True, seed=seed)
mask_datagen.fit(mask, augment=True, seed=seed)

if DEBUG:
    # save the images generated by image augmentation
    dmap_generator = dmap_datagen.flow(dmap, seed=seed, batch_size=batch_size,
                                   save_to_dir='/tmp/preview', save_prefix='dmap_', save_format='jpeg')
    im_generator = im_datagen.flow(im, seed=seed, batch_size=batch_size,
                                   save_to_dir='/tmp/preview', save_prefix='im_', save_format='jpeg')
    mask_generator = mask_datagen.flow(mask, seed=seed, batch_size=batch_size,
                                       save_to_dir='/tmp/preview', save_prefix='mask_', save_format='jpeg')
else:
    im_generator = im_datagen.flow(im, y=seg, seed=seed, batch_size=batch_size)
    mask_generator = mask_datagen.flow(mask, y=None, seed=seed, batch_size=batch_size)

# generate the augmented data
im_augmented, dmap_augmented = im_generator.next()

# combine generators into one which yields image and masks
train_generator = zip(im_generator, dmap_generator)
