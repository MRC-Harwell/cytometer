"""
This script aggregates all the cells in each '*_exp_0007_auto.json' file, thresholds them according to the field
cell_prob, saves the valid ones to a '*_exp_0007_auto_aggregate.json', and then creates a soft links to it that will be
read by AIDA.
"""
...
auto_filename_suffix = '_exp_0007_auto.json'
...
def process_annotations(annotation_files_list, overwrite_aggregated_annotation_file=False, create_symlink=False):
    ...
    for annotation_file in annotation_files_list:
        ...
        histo_file = os.path.basename(annotation_file).replace(auto_filename_suffix, '.tif')
        histo_file = os.path.join(histology_dir, histo_file)

        im = openslide.OpenSlide(histo_file)
        xres = 1e-2 / float(im.properties['tiff.XResolution']) * 1e6  # um
        yres = 1e-2 / float(im.properties['tiff.YResolution']) * 1e6  # um
        ...
        if not os.path.isfile(aggregated_annotation_file) or overwrite_aggregated_annotation_file:
            ...
            if DEBUG:
                plt.clf()
                plt.hist(np.array(areas) * 1e-3 * xres * yres, bins=500, density=True)
                plt.xlim(0, 29)
            ...
            if DEBUG:
                plt.clf()
                plt.hist(np.array(props['cell_prob']), bins=500, density=True)
                plt.xlim(0, 29)
                plt.xlabel('Area ($\mu m^2$)')
        ...
        if create_symlink:
            ...
            symlink_name = os.path.basename(histo_file).replace('.tif', '.json')
