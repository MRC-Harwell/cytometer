'''
Confidence measure for white adipocytes
'''
...
for i_fold, idx_test in enumerate([idx_test_all[0]]):
    ...
    train_dataset, train_file_list, train_shuffle_idx = \
        cytometer.data.load_datasets(im_train_file_list, prefix_from='im', prefix_to=['im', 'mask'],
                                     nblocks=nblocks, shuffle_seed=i_fold)
    test_dataset, test_file_list, test_shuffle_idx = \
        cytometer.data.load_datasets(im_test_file_list, prefix_from='im', prefix_to=['im', 'mask'],
                                     nblocks=nblocks, shuffle_seed=i_fold)
    ...
    # convert mask weights to categorical
    train_dataset['mask'] = keras.utils.to_categorical(train_dataset['mask'].astype(np.uint8))
    test_dataset['mask'] = keras.utils.to_categorical(test_dataset['mask'].astype(np.uint8))
